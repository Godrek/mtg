...
use mtg_gto::solver::mcts::{self, MctsConfig, MctsCampaignCheckpoint};
...
fn main() {
    ...
    if let Some(ref dir) = checkpoint_dir {
        println!("Checkpoint: {} (every {} games)", dir, checkpoint_every);
    }
    println!();

    if let Some(ref dir) = checkpoint_dir {
        // ── Checkpoint mode: run with save/resume ────────────────────────
        if is_commander {
